...
describe("sanitizeEmailHtml", () => {
  describe("dangerous tag removal", () => {
    it("removes script tags and their content", () => {
      const result = sanitizeEmailHtml('<script>alert("xss")</script><p>Hello</p>');
    ...
    it("removes style tags and their content", () => {
      const result = sanitizeEmailHtml("<style>body { color: red; }</style><p>Hello</p>");
    ...
    it("removes noscript tags", () => {
      const result = sanitizeEmailHtml("<noscript><p>No JS</p></noscript><p>Text</p>");
  ...
  describe("event handler removal", () => {
    ...
    it("removes onerror attributes", () => {
      const result = sanitizeEmailHtml('<img src="https://example.com/img.png" onerror="alert(1)" />');
      expect(result).not.toContain("onerror");
    });

    it("removes onload attributes", () => {
      const result = sanitizeEmailHtml('<body onload="steal()"><p>Hi</p></body>');
      expect(result).not.toContain("onload");
    });

    it("removes onmouseover attributes", () => {
      const result = sanitizeEmailHtml('<a href="https://safe.com" onmouseover="track()">Link</a>');
    ...
    it("removes all on* event handlers", () => {
      ...
      for (const handler of handlers) {
        const result = sanitizeEmailHtml(`<div ${handler}="evil()">Content</div>`);
  ...
  describe("safe inline styles", () => {
    it("preserves inline style attributes", () => {
      const result = sanitizeEmailHtml('<p style="color: blue; font-size: 14px;">Styled</p>');
  ...
  describe("image src filtering", () => {
    ...
    it("allows cid: image sources (inline attachments)", () => {
      const result = sanitizeEmailHtml('<img src="cid:image001@example.com" />');
    ...
    it("strips src for unknown protocol images", () => {
      const result = sanitizeEmailHtml('<img src="blob:https://example.com/abc" />');
